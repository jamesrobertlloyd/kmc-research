...
def test_rbm(learning_rate=0.1, training_epochs=15,
    ...
    for idx in xrange(n_samples):
        ...
        image_data[29 * idx:29 * idx + 28, :] = tile_raster_images(
                ...
                tile_spacing=(1, 1))
        image_mf_list.append(vis_mf)
        image_sample_list.append(vis_mf)
        # construct image

    # save the images

    with open('images_mf_saved.p', 'w') as save_file:
        cPickle.dump(image_mf_list, save_file)

    with open('images_sample_saved.p', 'w') as save_file:
        cPickle.dump(image_sample_list, save_file)
    ...
    # persistent_vis_chain = theano.shared(numpy.asarray(
    #         train_set_x.get_value(borrow=True)[train_idx:train_idx + n_chains],
    #         dtype=theano.config.floatX))
