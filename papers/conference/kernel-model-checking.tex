...
A constrasting method of Bayesian model criticism is the calculation of posterior predictive $p$-values \citep[e.g.][]{Rubin1984-tw}\fTBD{Find older references - See Bayarri Berger 2007 - Guttman} $p_\textrm{post}$ where the prior predictive distribution is replaced with the posterior predictive distribution ${Y \sim \int p(Y \given \theta, M)p(\theta \given Y^\textrm{obs}, M) \mathrm{d}\theta}$.
...
\TBD{Currently performing a $p$-values by depth experiment - unfortunately the parameters of the algorithms are very sensitive to depth so I need to make the experiments select these parameters ideally\ldots}
...
In this work the authors compare several methods for constructing Gaussian process\fTBD{cite GPML} regression models.
...
To avoid problems of multiple comparisons values in  bold indicate a positive discovery after a Benjamini--Hochberg\fTBD{cite} procedure with a false discovery rate of 0.05.
...
Figure~\ref{fig:SE-witness} shows the solar and gas production data sets\fTBD{Add the posterior to these plots} and the witness functions of the SE fits to these data.
...
\caption{
From left to right. Solar data. Witness function of SE fit to solar. Gas production data. Witness function of SE fit to gas production.
}
...
Figure~\ref{fig:ABCD-witness} shows the unemployment and Internet data sets and the witness functions of the ABCD fits to these data.
...
\caption{
From left to right. Unemployment data. Witness function of ABCD fit to unemployment. Internet data. Witness function of ABCD fit to Internet.
}
...
This claimed inadequacy of posterior predictive $p$-values was rebutted \citep{noauthor_undated-xk} and while their usefulness is becoming more accepted (see \eg introduction of \cite{Bayarri2007-cp}) it would appear there is still confusion on the subject \citep{Gelman2013-am}.
...
We have demonstrated an adaptation of the test to regression models and two sample tests for data with different symmetries (\NA{cite something related to functional two sample testing}) can be found in the literature.
