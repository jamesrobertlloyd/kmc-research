...
We then apply the procedure to real data where the models being assessed are restricted Boltzmann machines, deep belief networks and Gaussian process regression and demonstrate ways in which these models fail to capture the properties of the data they are trained on.
\end{abstract} 

\allowdisplaybreaks

\fTBD{DD, DR: Be clear about why the supremum is a good thing - it is still a statistic}
...
Model criticism typically proceeds by choosing a statistic of interest, computing it on data and comparing this to a suitable null distribution\fTBD{DR: Surely the sampling distribution DD: Explain}.
...
We demonstrate this approach to model criticism on toy examples, restricted Boltzmann machines and deep belief networks trained on MNIST digits and Gaussian process regression models trained on several time series.
...
A tail-area $p$-value can then be constructed for any statistic $T$ of the data\fTBD{Might be worth pointing out that a pivot will be independent of the prior}
...
We assume that our data $Y$ are \iid samples from some distribution ${(y_i)_{i=1\ldots n} \simiid p(y\given \theta, M)}$.
...
A way of answering the two sample problem is to consider maximum mean discrepancy (MMD) \citep[e.g.][]{Gretton2008-ik} statistics
\begin{equation}
...
\end{equation}
\ie the empirical witness function is the difference of two kernel density estimates \citep[e.g.][]{Rosenblatt1956-hx, Parzen1962-hk}.\fTBD{DD, JL: Make it clear that we may have arrived at a particular statistic but how it was defined is interesting}
...
This is not a failure; a test that can identify too many types of discrepancy would have low statistical power (see \citep{Gretton2008-ik} for discussion of the power of the MMD test and alternatives).\fTBD{Potentially work on this a bit more}
...
\emph{Right}: Witness function of MMD two sample test.
The erroneously fit cluster is clearly identified.
}
\label{fig:high_mog}
\end{figure}
...
In agreement with this observation, as well as computing estimates of marginal likelihoods and testing errors, it has been standard to demonstrate the effectiveness of a generative neural network by generating samples from the distribution it has learned.
...
\footnotetext{
Specifically these are the activations of the visible units before sampling sampling binary values.
This procedure is an attempt to be consistent with the grayscale input distribution of the images.
...
c) Troughs of the witness function for samples from 1500 RBMs (with differently initialised pseudo random number generators during training).
...
The estimated $p$-values were again all less than 0.001 and the summaries of the troughs of the witness function are shown in figure~\ref{fig:digits}c.
...
Summaries of the witness function troughs are shown in figure~\ref{fig:digits}d.
...
For comparison, figure~\ref{fig:digits}f shows digits closest to the peaks of the witness function; there is no trace of ghosting.
...
However this joint distribution information is rarely used; typically only the pointwise conditional distributions ${p(y\given x_i^\textrm{obs}, \hat\theta)}$ are used as we have done here.}
Table~\ref{table:ABCD-p-values} shows a table of $p$-values for 13 data sets and 4 regression methods.
The four methods are linear regression (Lin), Gaussian process regression using a squared exponential kernel (SE), spectral mixture kernels \citep{WilAda13} (SP) and the method proposed in \cite{Lloyd2014-ABCD} (ABCD).
...
%Lin, TCI and SP have a very similar pattern of significant $p$-values whereas ABCD has fewer significant $p$-values.
...
\caption{
\emph{From left to right}: Solar data with SE posterior. Witness function of SE fit to solar. Gas production data with SE posterior. Witness function of SE fit to gas production.
}
...
\caption{
\emph{From left to right}: Unemployment data with ABCD posterior. Witness function of ABCD fit to unemployment. Internet data with ABCD posterior. Witness function of ABCD fit to Internet.
}
...
We have demonstrated an example of MMD two sample tests to model criticism, but they can also be applied to any aspect of statistical modelling where two sample tests are appropriate.
...
We have demonstrated an adaptation of the kernel MMD test to regression models but investigating extensions to a greater number of model classes would be a profitable area for future study.
