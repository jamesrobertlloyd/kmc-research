...
In the case of exchangeable\fTBD{Make a forward pointer to how this can be applied to non-exchangeable data} data this null distribution can be replaced with ${(y_i)_{i=1\ldots n} \simiid p(y^\textrm{rep}|M,Y)}$ \ie that the data are \iid samples from the posterior predictive distribution over data points.
