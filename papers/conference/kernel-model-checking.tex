...
MMD tests not only provide a hypothesis test, but also show where any discrepancies are most extreme via their witness function.
...
Ideally these statistics are chosen to assess the utility of the statistical model under consideration (see applied example \eg \NA{cite Meulders 1998 and others}) but this can require considerable expertise on the part of the modeller.
...
Posterior predictive $p$-values (\NA{cite Rubin 1984, Meng 1994}) are defined by
\begin{equation}
...
We can test this null hypothesis using a two sample test (\NA{cite something generic}).
...
In response, we consider using dimensionality reduction techniques (\NA{classic citation?}) before performing two sample tests.
...
We mention this as a reminder that model checking cannot assess the `truth' of a model, only whether the data could plausibly have been generated by the model (or if the data is not extreme under the model if using posterior predictive $p$-values).
...
We generated 3000 independent samples from the learned generative model by initialising the network with a random training image and performing 1000 clamped gibbs updates\footnotemark~to generate each image (as in \eg \cite{Hinton2007}).
...
\TBD{Check out the bibliographic notes in BDA. This section generally needs work.}

\paragraph{Omnibus methods for model checking}

When Gelman et alia (\NA{cite G, M \& S}) introduced posterior predictive discrepancy measures, they also proposed an omnibus test discrepancy measure
\begin{equation}
...
For a recent survey of Bayesian methods for model assessment, selection and comparison see (\NA{cite Swedes}) which phrases many techniques as estimates of the utility of a model.
For some discussion of sensitivity analysis see (\NA{cite BDA or something better}).

In the spirit of scientific falsification (\NA{cite philosophy of science book?}), ideally all methods of assessing a model should be performed to gain confidence in any conclusions made that cannot be directly checked in some other manner.

\TBD{O'Hagan 2001 looks at KL divergences between prior and posterior (a divergence indicates a lack of sensitivity on the prior - which is intriguingly interpreted as a good thing).}

\paragraph{Other ways to use data for training and criticism}

\TBD{Prior predictive, posterior predictive, held out set and cross validated (cross validated cite Gelfand Dey 1992 also @incollection{gelfand1996model,
...
\TBD{Maybe say that van der Vaart shows that Bayarri and Berger, and prior predictive $p$-values of Box 1980, 1983.}
Alternatives have been proposed (\NA{cite Bayarri and Berger, Johnson 2004, 2006, Dey \& Gelfand}) that are similar to posterior predictive checks but are calibrated in the sense discussed in (van der Vaart).
...
\TBD{Perhaps the confusion arises due to the different use of the word model in frequentist circles and Bayesian circles? O'Hagan 2001 (HSSS) thinks it worth stating this in an introduction.}
...
In light of this observation the test proposed here may be most appropriate when trying to perform a Bayesian analysis that uses reference priors (\NA{cite something}) where the priors are chosen to minimise the impact on the posterior.
...
This includes Geweke tests of markov chain sampler validity (\NA{cite}) and markov chain convergence (\NA{cite some things}).
...
While two sample tests exist for data with different symmetries (\NA{cite something related to functional two sample testing}) it is unclear whether maximum mean discrepancy measures can be naturally extended to these classes.
