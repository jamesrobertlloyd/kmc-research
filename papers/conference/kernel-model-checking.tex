...
Consequently\fTBD{Need to work on wording} it is our belief that more effort shoud be expended on attempting to falsify models fitted to data, using model criticism techniques or otherwise.
Not only will this aid research in targeting areas for improvement but it should give greater confidence in any conclusions drawn from the models.
...
This is typically performed by constructing a statistic $T$ of the data whose distribution does not depend on the parameters $\theta$ \ie a pivotal quantity.
...
Analogous quantities in a Bayesian context are the prior predictive $p$-values of Box \citep{Box1980-ud}.
...
Both of these procedures quantify how surprised one should be after observing data $Y^\textrm{obs}$ having expected it to have been generated by the `model'.
The difference in methodologies comes from the differen use of the word `model'.
To a frequentist a model is a class of probability distributions over data indexed by parameters.
In contrast, a Bayesian model is a joint probability distribution over data and parameters.

\subsection{Criticising estimated models or posterior distributions}

A constrasting method of Bayesian model criticism is the calculation of posterior predictive $p$-values \citep[e.g.][]{Rubin1984-tw}\fTBD{Find older references - Dempster perhaps} where the prior predictive distribution in the null hypothesis is replaced with the posterior predictive distribution
\begin{equation}
  p_\textrm{post}(Y^\textrm{obs}) = \mathbb{P}(T(Y)\geq T(Y^\textrm{obs})) \quad \textrm{where} \quad Y \sim \int p(Y \given \theta, M)p(\theta \given Y^\textrm{obs}, M) \mathrm{d}\theta.
  \label{eq:post-p-value}
\end{equation}

The corresponding test for an analysis resulting in a point estimate of the parameters $\hat\theta$ would replace the posterior predictive distribution with the plug-in predictive distribution $p(Y \given \hat\theta, M)$ to form the plug-in $p$-value $p_\textrm{plug}$.
These $p$-values quantify how surprised one should be if, after performing inference after observing data $Y^\textrm{obs}$, one were to observe new data whose value of the statistic $T$ was equal to that of the original data.

\subsection{Which type of model criticism should be used?}

Different forms of model criticism are appropriate in different contexts, but we believe that posterior predictive and plug-in $p$-values will be most often useful for the types of statistical model considered in machine learning.
For example, suppose one is fitting a nonparametric or otherwise very flexible model to data \eg a deep belief network.
Classical and prior predictive $p$-values would assume the null hypothesis that the data could have been generated from some deep belief network.
Since the space of possible deep belief networks is very large it will be difficult to ever falsify this hypothesis.
A more interesting null hypothesis to test in this context is whether or not our particular trained deep belief network can faithfully mimick the distribution it was trained on.

\section{Model criticism for \iid data using two sample tests}
\label{sec:model-crit}

We now consider the situation where we can assume that our data $Y$ are \iid samples from some unknown distribution $(y_i)_{i=1\ldots n} \simiid p(y\given \theta, M)$.
The null hypothesis associated with a plug-in $p$-value is then $(y^\textrm{obs}_i)_{i=1\ldots n} \simiid p(y \given \hat\theta, M)$.

We can test this null hypothesis using a two sample test \citep[e.g.][]{Friedman1979-ur, Bickel1969-ao, Hotelling1951-jd}.
In particular, we have samples of data $(y_i)_{i=1\ldots n}$ and we can generate samples from the plug-in predictive distribution $(y_i^\textrm{rep})_{i=1\ldots m}$ and then ask whether or not these samples could have been generated from the same distribution.
...
This is in contrast with the $p$-values introduced thus far which ask if the data is extreme as measured by a statistic $T$.
...
These types of statistics are of interest since they are computed at a function that extremises the mean discrepancy between two distributions.
...
From this final equation we can see that the empirical witness function is the difference of two kernel density estimates\fTBD{Cite Nadaraya and Watson}.
...
There are a number of different ways in which the null distribution of the MMD statistic\fTBD{Cite equation} can be estimated \citep[e.g.][]{Gretton2008-ik}.
...
We can therefore speculate that the data is more leptokurtic\fTBD{ZG: Does not follow} (has heavier tails) than the fitted model.
...
Subsequently\fTBD{ZG: Very old practice}, as well as computing estimates of marginal likelihoods and testing errors, it became\fTBD{ZG: Very old practice} standard to demonstrate the effectiveness of a neural network by generating samples from the distribution it had learned\fTBD{cite things}.
...
We generated 3000 independent samples from the learned generative model by initialising the network with a random training image and performing 1000 clamped\fTBD{ZG: Clarify that you are clamping labels, not pixels} gibbs updates\footnotemark~to generate each image (as in \eg \cite{Hinton2007-eo}).
...
\fTBD{This can probably be removed - was created when I still had conceptual misunderstandings}
