...
A constrasting method of Bayesian model criticism is the calculation of posterior predictive $p$-values \citep[e.g.][]{Rubin1984-tw}\fTBD{Find older references - Dempster perhaps} $p_\textrm{post}$ where the prior predictive distribution in the null hypothesis is replaced with the posterior predictive distribution ${Y \sim \int p(Y \given \theta, M)p(\theta \given Y^\textrm{obs}, M) \mathrm{d}\theta}$.
...
\TBD{Currently performing a $p$-values by depth experiment - will hopefully have a nice table of $p$-values}

\subsection{Testing non \iid data}

It is a bit restrictive to stick to \iid.
Do we have any hope of applying in non \iid case.
Remember that we only need a $p$-value.
Regression is kinda on the border line - surely we can do something - let's make a suggestion.

\subsubsection{A nonparametric test of heteroscedasticity and non-normality for regression}

Naive thing would be a little slow - requiring a potentially costly estimation of density estimate.
Let's try something cheaper and see what happens.
...
\fTBD{ZG: Discussion of related work feels disconnected JL: Needs to be updated anyway}
