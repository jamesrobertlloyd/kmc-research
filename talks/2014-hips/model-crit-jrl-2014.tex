...
\begin{frame}{Science}
  ...
  \begin{tikzpicture}[node distance = 0.3\textwidth]
    ...
    \node [block] at (0.4\textwidth, -0.2\textwidth) (apply) {Application};
...
\begin{frame}{Many ML papers stop at evaluation\dots}
  \begin{itemize}
    \item Easy to positively review a paper which demonstrates improved cross validated predictive performance\dots
    \vspace{\baselineskip}
    \pause
    \item \dots so many research efforts stop when they can demonstrate improved predictive performance
    \vspace{\baselineskip}
    \pause
    \item At its worst this is the equivalent of $p$-value hacking
    \vspace{\baselineskip}
    \pause
    \item Predictive performance alone tells us very little about how to improve a model / method    
  \end{itemize}
\end{frame}

\begin{frame}{\dots breaking the inductive part of the loop}
  \begin{center}
  \tikzstyle{block} = [rectangle, draw, fill=blue!20, 
                       text width=0.2\textwidth, text centered, rounded corners, minimum height=3em]
  \tikzstyle{line} = [draw, -latex', ultra thick]
